...
\subsection{Constant Evaluation and Control Flow Analysis} { The compiler can now replace expressions consisting of literals with a literal equal to the result of the expression. This applies to both expressions and sub-expressions. The compiler is also able to use lazy evaluation on boolean expressions contain boolean literals. This process occurs automatically during compilation.
...
\subsection{Structs} { We also extended the compiler to support C-Style structs. These are user defined data types declared at the head of the WACC file by combining a set of previously defined or primative types. Importantly structs have a fixed size at compile time, therefore the only types which they can contain are the primitive, pointer and other struct types which are of fixed size, this automatically allows pair types to be stored since pairs in WACC are defined to be pointers. Array types required the most consideration in how they would be stored within a struct, ultimately since we had already extended WACC to use dynamic arrays we opted to not allow structs to store arrays in place but rather to store a pointer to their head. Structs in WACC act very much like C structs, with assignment via copying rather than reference and inline initialisation, they only differ in that they must be assigned a value when they are initialised (although this value may be null), this was done mainitain a consistency of style within WACC since all other types must be assigned a value on initialisation and to break this style would confuse a user of the language.       

}
\subsection{Reflection and future possible extensions} {When we began the extension section of the project we set our aspirational goal as implementing classes with generic typing along with function pointers to produce a library of generic higher order functions. Obviously we did not reach this goal, however in our planning we did not fully expect to. We planned the extension in a modular fashion, structs were a requirement for classes, importing dependencies was a requirement for writing function libraries and function pointers and generics were requirements for higher-order functions. By organising the project in this way we were able to make meaningful progress along each of the branches required for for our final goal and adding significantly to the functionality of the compiler despite the fact that we did not fully complete all of our objectives in the extension. One unfortunate effect of dividing work in this way was that it forced members of the group to work more separately from one another than in the previous sections of the project, meaning that inconsistencies between branches grew much more common, with persistent merge conflicts and time wasted on unexplainable bugs. If we were to extend the compiler further we would obviously continue along completing the objectives we already set, however we would take care to communicate the addition of any new feature and merge group work more often.}
